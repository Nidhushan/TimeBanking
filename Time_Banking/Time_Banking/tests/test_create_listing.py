...
class CreateListingViewTests(TestCase):
    ...
    def test_create_listing_with_valid_data(self):
        ...
        self.assertQuerySetEqual(
            response.json()["tags"], [str(self.tag1), str(self.tag2)], ordered=False
        )
    ...
    def test_create_listing_with_random_data(self):
        ...
        for _ in range(10):
            ...
            description = "This is a test description " + str(
                random.randint(1, 10000000000)
            )
            listing_type = random.choice(["Offer", "Request"])
            ...
            self.assertEqual(
                response.json()["duration"], str(timedelta(minutes=int(duration)))
            )
            self.assertQuerySetEqual(
                response.json()["tags"], [str(self.tag1), str(self.tag2)], ordered=False
            )
    ...
    def test_create_listing_with_missing_required_fields(self):
        ...
        for i in range(6):
            ...
            while titles in used_titles:
                titles = "Test Listing with missing field" + str(
    ...
    def test_create_listing_with_duration(self):
        ...
        for duration in invalid_duration_field:
            ...
            while titles in used_titles:
                titles = "Test Listing with invalid duration" + str(
                    random.randint(1, 1000)
                )
            used_titles.append(titles)
            data = {
        ...
        for duration in right_duration:
            titles = "Test Listing  with different duration " + str(
                random.randint(1, 1000)
            )
            while titles in used_titles:
                titles = "Test Listing with different duration " + str(
    ...
    def test_create_listing_with_too_frequent_requests(self):
        ...
        for i in range(10):
            titles = "Test Listing with too frequent test" + str(
        ...
        self.assertIn("error", response.json())
        self.assertEqual(
            response.json()["error"], "You are creating services too quickly."
        )
        Listing.objects.all().delete()
    ...
    def test_create_listing_with_same_title(self):
        ...
        self.assertEqual(
            response.json()["error"],
            "You are trying to create a service with duplicated title.",
        )
        Listing.objects.all().delete()
    ...
    def test_create_listing_with_too_many_listings(self):
        ...
        used_titles = []
        for i in range(301):
            titles = "Test Listing with too many listings" + str(
                random.randint(1, 1000000)
            )
            while titles in used_titles:
                titles = "Test Listing with too many listings" + str(
        ...
        self.assertIn("error", response.json())
        self.assertEqual(
            response.json()["error"], "You have reached the maximum number of services."
        )
        Listing.objects.all().delete()
    ...
    def test_create_listing_exception_handling(self):
        with mock.patch(
            "Time_Banking.models.Listing.objects.create",
