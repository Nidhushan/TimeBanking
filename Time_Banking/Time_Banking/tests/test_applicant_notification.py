...
from ..models import Category, Listing, ListingResponse, Notification, Tag, User
...
class NotificationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.notification = Notification.objects.create(
            user=self.user, message="Test Notification", is_read=False, url="/test-url"
        )
    ...
    def test_get_notifications(self):
        ...
        self.assertIn(
            "Test Notification", response.json()["notifications"][0]["message"]
        )

    def test_mark_as_read(self):
        response = self.client.post(
            reverse("mark_as_read", args=[self.notification.id])
        )
        self.assertEqual(response.status_code, 200)
...
class ViewApplicantsTests(TestCase):
    ...
    def setUp(self):
        ...
        self.response1 = ListingResponse.objects.create(
            user=self.user, listing_id=1, status=0, message="Initial"
        )
        self.response2 = ListingResponse.objects.create(
            user=self.other_user, listing_id=1, status=0, message="Initial"
        )
    ...
    def test_view_applicants_post(self):
        ...
        response = self.client.post(
            reverse("view_applicants", args=[1]), data=post_data
        )
        self.assertEqual(response.status_code, 302)  # Redirec
        ...
        self.assertEqual(
            notification.message, "You get an update on your applied service."
        )
...
class AppliedServicesTests(TestCase):
    ...
    def setUp(self):
        ...
        self.response = ListingResponse.objects.create(
            user=self.user, listing_id=1, status=1, message="Test response"
        )
