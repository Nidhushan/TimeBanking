...
LISTING_TYPES = [(True, "Offer"), (False, "Request")]
...
class Listing(models.Model):
    LISTING_TYPES = [("Offer", "Offer"), ("Request", "Request")]
    ...
    tags = models.ManyToManyField(
        Tag
    )  # backend needs to check that tags belong to category
    title = models.CharField(max_length=50)  # use CharField (adjust length as needed)
    description = models.TextField(
        max_length=1000
    )  # bound to 1000 characters (adjust as needed)
    ...
    )  # how long the work is expected/preferred to take
    ...
    def clean(self):
        valid_statuses = (
            STATUS_CHOICES_OFFER if self.listing_type else STATUS_CHOICES_REQUEST
        )
...
class ListingResponse(models.Model):
    ...
    )  # TODO: map integers to types, eg. 1 = accept, 2 = reject, etc.
