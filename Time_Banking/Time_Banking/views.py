...
from .models import Category, Listing, ListingResponse, Notification, Skill, Tag
...
def resend_verification_email(request):
    ...
    if user_data:
        ...
        return redirect(
            "create_account"
        )  # If user data doesn't exist, redirect to account creation
...
def create_account(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "verify_account_code"
            )  # Redirect to verification code entry
        else:
            return render(request, "create-account.html", {"form": form})
    ...
    if request.method == "POST":
        try:
            ...
            if (
                ...
                return JsonResponse(
                    {"error": "Invalid input, missing fields"}, status=400
                )

            if username != request.user.username:
                return JsonResponse(
                    {"error": "Not authorized to change password"}, status=403
                )
            ...
            # Check if the current password is correc
...
def apply_service(request, listing_id):
    if request.method == "POST":
        ...
        if listing.creator == request.user:
            return JsonResponse(
                {"error": "You cannot apply your own service/request."}, status=403
            )

        # Check for duplicate application
        if ListingResponse.objects.filter(listing=listing, user=request.user).exists():
            return JsonResponse(
                {"error": "You have already applied to this listing."}, status=400
            )
        ...
        return JsonResponse(
            {"success": "Service/Request apply successfully!"}, status=200
        )
...
def create_listing(request):
    if request.method == "POST":
        try:
            ...
            listing_type = request.POST.get(
                "listing_type"
            )  # Expecting 'True' or 'False'
            duration_in_minutes = request.POST.get(
                "duration"
            ...
            try:
                ...
                return JsonResponse(
                    {"error": "Duration must be a valid integer"}, status=400
                )

            # check listing is valid to create or no
            user_listings_old = Listing.objects.filter(creator=request.user)
            if len(user_listings_old) > 0:
                if len(user_listings_old) > 299:
                    return JsonResponse(
                        {"error": "You have reached the maximum number of services."},
                        status=400,
                    )
                for user_listing in user_listings_old:
                    if user_listing.title == title:
                        return JsonResponse(
                            {
                                "error": "You are trying to create a service with duplicated title."
                            },
                            status=400,
                        )
            ...
            if not getattr(
                settings, "DISABLE_RATE_LIMIT_CHECK", False
            ):  # Disable rate limit check for test cases
                if len(user_listings_old) > 0:
                    if latest_listing_posted_at > listing.posted_at - timedelta(
                        seconds=30
                    ):
                        listing.delete()
                        return JsonResponse(
                            {"error": "You are creating services too quickly."},
                            status=429,
                        )
...
def edit_listing_page(request, listing_id):
    ...
    if listing.creator != request.user:
        return JsonResponse(
            {"error": "You can only edit your own service/request."}, status=403
        )
